...
void popRiwayat() {
    if (top == -1) {
        ...
        cout << "[Batal] Transaksi terakhir atas nama " << (riwayat + top)->namaPenumpang << " dihapus.\n";
        top--;
    }
}
...
void peek() {
    cout << "\n--- PEEK DATA ---" << endl;
    if (front != -1 && front <= rear)
        cout << "Depan Antrean: " << (antrean + front)->namaPenumpang << endl;
    else
        cout << "Depan Antrean: (Kosong)" << endl;

    if (top != -1)
        cout << "Riwayat Terakhir: " << (riwayat + top)->namaPenumpang << endl;
    else
